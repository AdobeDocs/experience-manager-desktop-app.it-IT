...
Risolvi i problemi relativi all’app desktop AEM per risolvere i problemi occasionali correlati all’installazione, all’aggiornamento, alla configurazione e così via.
...
Con questa architettura, l’app desktop intercetta le chiamate al file system (apertura, chiusura, lettura, scrittura e così via) alla condivisione di rete montata e le traduce in chiamate HTTP native di AEM al server AEM. I file vengono memorizzati nella cache locale. Per ulteriori dettagli, vedi [Utilizzare l&#39;app desktop AEM v1.x](use-app-v1.md).
...
* Un utente accede ad AEM e utilizza l’interfaccia web per individuare una risorsa.
* Utilizzando le funzionalità di azione desktop dell’interfaccia utente web di AEM, l’utente apre, visualizza o modifica la risorsa sul desktop in base alle esigenze.
...
Adobe consiglia di eseguire alcuni test dei casi d’uso prima di confermare a un client che alcuni tipi di file possono essere modificati in modo efficiente direttamente dalla directory montata.
...
Se un file locale non è coerente con la versione disponibile nel server, la finestra di dialogo dello stato del caricamento in background notifica il conflitto. Per risolvere il problema, aprire il file in conflitto e salvarlo. Il salvataggio del file costringe AEM Desktop a sincronizzare le modifiche locali più recenti con AEM. Puoi visualizzare le versioni precedenti della risorsa nella timeline e risolvere eventuali conflitti.

Considera fattori aggiuntivi quando più utenti tentano di lavorare in directory montate separate che hanno come destinazione la stessa istanza di AEM. In particolare, sono importanti i seguenti fattori:
...
Se le prestazioni di WebDAV/SMB diminuiscono drasticamente quando più utenti lavorano contemporaneamente, è possibile configurare alcuni elementi in AEM per migliorare le prestazioni.

## Aggiornare i flussi di lavoro transitori delle risorse {#update-asset-transient-workflows}

Puoi migliorare le prestazioni di AEM abilitando flussi di lavoro transitori per il flusso di lavoro Aggiorna risorsa DAM. L’abilitazione di flussi di lavoro transitori riduce la potenza di elaborazione necessaria per aggiornare le risorse quando vengono create o modificate in AEM.

1. Passare a `/miscadmin` nell&#39;istanza di Experience Manager (`https://[aem_server]:[port]/miscadmin`).
...
Un altro metodo per migliorare le prestazioni di AEM consiste nel configurare il valore del numero massimo di processi paralleli per il processo Coda flusso di lavoro transitorio Granite. Il valore consigliato è circa la metà del numero di CPU disponibili con il server. Per regolare il valore, effettuare le seguenti operazioni:

1. Passare a `/system/console/configMgr` nell&#39;istanza di AEM da configurare, ad esempio `https://[aem_server]:[port]/system/console/configMgr`.
...
A causa dei limiti di larghezza di banda della rete, le prestazioni di WebDAV/SMB possono peggiorare quando più utenti lavorano contemporaneamente. Per migliorare le prestazioni di WebDAV/SMB, Adobe consiglia di aumentare le dimensioni dell&#39;istanza AWS per un&#39;istanza AEM di destinazione eseguita su AWS.
...
* Se davanti all’istanza di AEM è presente un Dispatcher, accertati che sia di dimensioni appropriate. Se l&#39;istanza di AEM fornisce 4000 Mbps ma il Dispatcher fornisce solo 500 Mbps, la larghezza di banda effettiva è di soli 500 Mbps. Questo perché Dispatcher crea un collo di bottiglia della rete.
...
Quando si scrive in un file estratto, il blocco viene applicato solo all&#39;interno dell&#39;implementazione AEM WebDAV. Di conseguenza, i client che utilizzano WebDAV, come l&#39;app desktop, applicano solo il blocco. Il blocco non viene applicato tramite l’interfaccia web di AEM. L’interfaccia di AEM mostra semplicemente un’icona a forma di lucchetto nella vista a schede per le risorse estratte. L’icona è cosmetica e non ha alcun effetto sul comportamento di AEM.
...
La sostituzione di un file non mostra alcun avviso o errore, ma il controllo della risorsa in AEM rivela che rimane invariata. Aggiorna o controlla la risorsa in AEM per verificare che non sia in fase di modifica.
...
AEM Desktop tenta di sincronizzare un determinato file tre volte. Se dopo il terzo tentativo la sincronizzazione del file non riesce, AEM Desktop considera il file in conflitto e invia una notifica tramite la finestra di stato del caricamento in background. Uno stato di conflitto indica che le modifiche più recenti sono ancora disponibili localmente, ma non vengono sincronizzate di nuovo in AEM. L’app desktop AEM non tenta più di eseguire la sincronizzazione.

Il modo più semplice per risolvere la situazione è aprire il file in conflitto e salvarlo nuovamente. Forza AEM Desktop a tentare la sincronizzazione per altre tre volte. Se la sincronizzazione del file non riesce, vedere le sezioni seguenti per ulteriori informazioni.

## Cancellazione della cache di AEM Desktop {#clearing-aem-desktop-cache}
...
Per cancellare la cache, elimina la directory &lt;Endpoint AEM codificato>.
...
Fai clic sull&#39;icona AEM Desktop, quindi scegli **Informazioni**. Sullo schermo viene visualizzato il numero di versione.
...
Indipendentemente dal comportamento, il file rimane invariato quando lo si archivia. Anche se viene visualizzata una versione diversa del file, le modifiche non vengono sincronizzate in AEM.

## Risoluzione dei problemi relativi allo spostamento dei file {#troubleshooting-problems-around-moving-files}

L’API server richiede l’invio di intestazioni aggiuntive, X-Destination, X-Depth e X-Overwrite, affinché le operazioni di spostamento e copia funzionino correttamente. Dispatcher non passa queste intestazioni per impostazione predefinita, il che causa il mancato funzionamento di queste operazioni. Per ulteriori informazioni, vedere [Connessione ad AEM dietro a Dispatcher](install-configure-app-v1.md#connect-to-an-aem-instance-behind-a-dispatcher).
...
1. Verifica che tutto ciò che precede `/content/dam.json` corrisponda al valore AEM di destinazione configurato in AEM Desktop.
...
AEM Desktop funziona con le implementazioni di AEM basate su un Dispatcher, che è una configurazione predefinita e consigliata per i server AEM. I dispatcher di AEM davanti agli ambienti di authoring di AEM sono generalmente configurati per saltare la memorizzazione nella cache delle risorse DAM. Pertanto, i dispatcher non forniscono un caching aggiuntivo dal punto di vista di AEM Desktop. Assicurati che la configurazione di Dispatcher sia regolata per funzionare con AEM Desktop. Per ulteriori dettagli, vedere [Connessione ad AEM con un Dispatcher](install-configure-app-v1.md#connect-to-an-aem-instance-behind-a-dispatcher).

## Controllo dei file di registro {#checking-for-log-files}

A seconda del sistema operativo in uso, è possibile trovare i file di registro di AEM Desktop nelle posizioni seguenti:
